...
def article(request, page):
    if page == '':
        page = 1
    page = int(page)
    objects = Article.objects.order_by('-article_date')[:1000]
...
def day_article(request, page):
    ...
    data = {'items': objects, 'current_page': page, 'page_number': total_page,
            'pages': pages}
    return render(request, 'News/day_article.html', data)


def month_article(request, page):
    if page == '':
        page = 1
    page = int(page)
    objects = Article.objects.order_by('-article_date')[:500]
    ...
    data = {'items': objects, 'current_page': page, 'page_number': total_page,
            'pages': pages}
    return render(request, 'News/month_article.html', data)

def year_article(request, page):
    if page == '':
        page = 1
    page = int(page)
    objects = Article.objects.order_by('-article_date')[:1000]
    ...
    data = {'items': objects, 'current_page': page, 'page_number': total_page,
            'pages': pages}
    return render(request, 'News/year_article.html', data)
